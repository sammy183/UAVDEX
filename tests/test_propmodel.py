...
class TestSimplifiedRPM(unittest.TestCase):
    ...
    def test_SimplifiedRPMBase_t(self):
        ...
        for Uinf in [10, 30, 50, 70, 90]:
            ...
            # Uinf/J = (RPM/60)*D
            # (Uinf*60)/(J*D) = RPM
            RPMmin = (Uinf*60)/(Jmax*d) # this provides the LOWER RPM feasibility boundary!!! 
            # print(RPMmin) 
            ...
            # let me ask another question. RPM only *drops* when SOC < 0?
            # therefore can't we just not care about it? YES
            ...
            if SimpleRPMeqsBase_t(RPMmin, t, Uinf, dT, rho, eta_c, eta_g, *args)[0] < RPMmin:
                print('this is infeasible')
            
            # ok that check is good
            # STILL a problem can emerge: when SOC = 0 is the trigger for the residual to = 0 (see cobra, 10 m/s case)
            # this problem is infeasible bc of t
            ...
            n = 1000
            RPMs = np.linspace(max(self.design.rpm_list.min(), RPMmin), self.design.rpm_list.max(), n)
